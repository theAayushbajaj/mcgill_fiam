...
**Important:** Perform all analysis internally and **output only the Readability Score as a number between 0 and 1. Do not include any explanations, reasoning, or additional text.**"""},
        ...
        readability_score_string = outputs[0]['generated_text'][-1]['content']
        try:
            readability_score = float(readability_score_string)
            results.append(max(0, min(readability_score, 1)))
        except Exception as e:
            print(f"Error {e} processing readability score: {readability_score_string}")
            results.append(None)

    return_dict = {key: samples[key] for key in samples.keys()}
    return_dict['READABILITY_SCORE'] = results
...
if __name__ == "__main__":
    ...
    # Process the dataset
    results = dataset.map(calculate_readability_score, batched=True, batch_size=128)
