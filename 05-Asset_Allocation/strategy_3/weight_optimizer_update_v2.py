...
def get_rec_bipart(cov, mu, c_items, lambda_, benchmark, soft_risk, w_c=1.0):
    # Base case: if cluster has one asset, assign the cluster weight to the asset
    if len(c_items) == 1:
        return pd.Series({c_items[0]: w_c})
    else:
        ...
        # Ensure scores are non-negative and avoid division by zero
        c1_score = max(c1_score, 0) + 1e-6
        c2_score = max(c2_score, 0) + 1e-6
